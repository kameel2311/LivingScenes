...
if __name__ == "__main__":
    ...
    for idx in range(1, object_index_limit):
        ...
        for view_idx in range(NUM_VIEWPOINTS - 1):
            ...
            R = []
            t = []
            for ref_object_pointcloud, rescan_object_pointcloud in zip(
                ref_object_pointclouds, rescan_object_pointclouds
            ):
                ref_object_pointcloud = torch.unsqueeze(
                    ref_object_pointcloud.T, dim=0
                ).double()
                rescan_object_pointcloud = torch.unsqueeze(
                    rescan_object_pointcloud.T, dim=0
                ).double()
                print("type: ", ref_object_pointcloud.dtype)
                print(ref_object_pointcloud.shape)
                est_R, est_t = solver._solve_pairwise_registration(
                    ref_object_pointcloud,
                    rescan_object_pointcloud,
                    optim=False,
                )
                R.append(est_R.squeeze(0))
                t.append(est_t.squeeze(0))
            R = torch.stack(R)
            t = torch.stack(t)
