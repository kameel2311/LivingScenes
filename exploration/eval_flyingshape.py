...
def eval_matching(dataset, solver):
    ...
    logging.info(
        f"Scene-level recall @25: {scene_recall25:.2f} | @50: {scene_recall50:.2f} | @75: {scene_recall75:.2f} | @100: {scene_recall100:.2f}"
    )
    return
...
def eval_relocalization(dataset, solver):
    ...
    for data in tqdm(dataset):
        ...
        rescan_pc_lst = [
            torch.from_numpy(scene["pc"]).cuda().float().transpose(-1, -2)
            for scene in data[1:2]
        ]

        ref_tsfm = torch.from_numpy(data[0]["transform"]).cuda().float()
        rescan_tsfm_lst = [
            torch.from_numpy(scene["transform"]).cuda().float() for scene in data[1:2]
        ]
        # with torch.no_grad():
        #     ref_code = model.encode(ref_pc)
        #     rescan_code_lst = [model.encode(rescan_pc) for rescan_pc in rescan_lst]
        ...
        for rescan_pc, rescan_tsfm in zip(rescan_pc_lst, rescan_tsfm_lst):
            gt_tsfm_lst = torch_se3.concatenate(
            ...
            for ins_ref, ins_rescan, gt_tsfm in zip(ref_pc, rescan_pc, gt_tsfm_lst):
                ...
                tsfm_err = compute_transformation_error(
                    ins_ref.T[None], ins_rescan.T[None], pred_tsfm, gt_tsfm
                )

                # compute chamfer distance (downsampled by 10 because there are so many points)
                chamfer_l1 = chamfer_distance_torch(
                    ins_ref.T[None], ins_rescan.T[None], pred_tsfm, gt_tsfm
                )
    ...
    logging.info(
        f"Recall [5 deg]: {rmse_recall:.2f} | RRE: {med_rre1:.2f} [deg] || RTE: {med_rte1:.2f} [m]"
    )
    ...
    logging.info(
        f"Recall [RRE<10deg]: {re_recall:.2f}| RRE: {med_rre2:.2f} [deg] || RTE: {med_rte2:.2f} [m]"
    )
    ...
    np.savez(
        osp.join(
            solver.cfg["shape_priors"]["ckpt_dir"], "summary/flyingshapes_dict_no_optim"
        ),
        rre_lst=rre_list,
        rte_lst=rte_list,
        tsfm_lst=tsfm_err_list,
        cd_lst=cd_lst,
    )
    return
    ...
    for data in tqdm(dataset):
        for t_scene in data[:1]:
            ...
            for i in range(pc.shape[0]):
                ...
                gt_mesh_path = osp.join(
                    "/home/liyuanzhu/projects/MA/data/intermediate/watertight",
                    t_scene["class_id"][i],
                    t_scene["obj_id"][i] + ".obj",
                )
                gt_mesh = trimesh.load_mesh(gt_mesh_path)
                if pred_mesh.vertices.shape[0] != 0:
                    cd1, cd2 = compute_chamfer_distance(
                        gt_mesh, pred_mesh, offset=0, scale=1
                    )
