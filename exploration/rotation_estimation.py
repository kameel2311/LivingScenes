...
if __name__ == "__main__":
    ...
    pointcloud_2, rotation = rotate_pointcloud_randomly(
        pointcloud_2, pure_z_rotation=True, about_center=True
    )
    pointcloud_2 = add_gaussian_noise(pointcloud_2, sigma=noise_std)
    gt_rotation = torch.tensor(rotation)

    object_pointclouds = (
        torch.tensor(np.array([pointcloud_1, pointcloud_2])).cuda().transpose(-1, -2)
    )
    ...
    draw_point_cloud(
        ...
        title="Aligned with Estimated Rotation",
    )
